...
class CosmosDBTests(ScenarioTest):
    ...
    def test_create_database_account(self, resource_group):
        ...
        self.cmd('az cosmosdb create -n {acc} -g {rg} --enable-automatic-failover --default-consistency-level ConsistentPrefix --netowrk-acl-bypass AzureServices --network-acl-bypass-resource-ids {network_acl_bypass_resource)id}')
        ...
        self.cmd('az cosmosdb update -n {acc} -g {rg} --enable-automatic-failover false --default-consistency-level Session --disable-key-based-metadata-write-access --enable-public-network false --netowrk-acl-bypass None' )
        self.cmd('az cosmosdb show -n {acc} -g {rg}', checks=[
